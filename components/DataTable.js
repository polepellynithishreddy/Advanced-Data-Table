...
import { Table, Thead, Tbody, Th, Tr, Td, Input } from '@material-ui/core';
...
const DataTable = () => {
  ...
  const filteredRows = rows.filter(row => {
    const { id, name, category, price, sale_price } = row.values;
    return (
      id.toString().includes(filterInput) ||
      name.toLowerCase().includes(filterInput.toLowerCase()) ||
      category.toLowerCase().includes(filterInput.toLowerCase()) ||
      (price && price.toString().includes(filterInput)) ||
      (sale_price && sale_price.toString().includes(filterInput))
    );
  });
